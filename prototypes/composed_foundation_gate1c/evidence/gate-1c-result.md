...
- SDK archive SHA-256:
  `a69fafd50800f8bf5a4c752494bea523a5ce0728eff196a2d04f440669a60547`
- SDK archive size: 127,813,381 bytes
- SDK archive entries: 12,119
- Manifest payload entries: 12,118, plus `manifest.json`
...
`/Brepro` removed that nondeterminism. Clean runs 8 and 9 then passed bytewise
`cmp` for both `falcor-sdk.zip` and `falcor_vulkan_smoke.exe`.
...
both contain `Falcor.dll` and `GFSDK_Aftermath_Lib.x64.dll`; the smoke bundle
also contains `gate1c.slang` and `run-smoke.ps1`.
