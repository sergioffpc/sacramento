...
Latest approved amendment: `OBS-CONTRACT-004` core-runtime observability reconciliation, project owner, 2026-09-04
...
**NFR-OBSERVABILITY-CORE-001** — The approved Observability Contract core inventory MUST contain process and Training Session lifecycle; Lifecycle Revision; Canonical Tick commit and Composite Confirmed-State Identity; final-image intervals; correlated action submission, authoritative-result receipt, and rendered presentation; acoustic-event initiation and presentation; Admission start and terminal result; AUTH Audit Commit Unit reference when the Production Security Baseline applies; Technical Removal ordering and affected canonical versions; Prediction correction, rollback or reset, and lead-bound freeze; replication acknowledgement high-water, delta gap, digest mismatch, and baseline resynchronization; catch-up or overload-bound breach and deterministic disposition; reconstruction-record loss classification and affected range; Session Evidence Set terminal correlation, completeness, and durable-handoff disposition; exact Application Release, Runtime Launch Specification, build, configuration, AUTH mode, applicable profile versions and Runtime Content Release; and counts of lost or discarded observability signals. Core signals MUST NOT contain gameplay payloads, state digests, credentials, authentication evidence, reusable proofs, Trainee Identity, Synthetic Identity, identity-bound performance fields, detailed AUTH denial reasons, or other personal data.
...
| `NFR-OBSERVABILITY-CORE-001` | Quality-support and data-minimization requirement | Approved; mandatory for the initial baseline | Automated NFR and authoritative-runtime evidence needs a closed minimum signal set, while information security outranks operational convenience | Test and production Desktop Mode and Session Authority builds | Inspection of the exact approved Observability Contract version and closed core inventory; Automated Test exercising every emission, absence, order, cardinality, correlation, timestamp-domain, loss, deterministic-disposition and `CoreOnly` case; comparison with authoritative reconstruction or immutable published views; and negative gameplay, identity, proof, digest, denial-reason and authority-feedback cases | None |
