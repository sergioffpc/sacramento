...
**NFR-OBSERVABILITY-CORE-001** — The Observability Contract core inventory MUST contain process lifecycle and readiness; final-image intervals; correlated action submission, authoritative-result receipt, and rendered presentation; acoustic-event initiation and presentation; Admission start, terminal result, and AUTH Audit Commit Unit reference; exact build, configuration, and applicable profile versions; and counts of lost or discarded observability signals. Core signals MUST NOT contain gameplay payloads, credentials, authentication evidence, or personal data.

**NFR-OBSERVABILITY-RETENTION-001** — In production, core operational signals MUST remain retrievable for at least 30 days after collection. Readiness-check results, declared-maintenance records, healthy-environment exclusion evidence, and their calculation inputs for `NFR-AUTHORITY-AVAILABILITY-001` MUST remain retrievable for at least 365 days after collection. Formal acceptance evidence remains governed by the verification plan rather than these operational retention periods.
...
**NFR-OBSERVABILITY-ALERTING-001** — In production, each increase in a core lost-or-discarded-signal counter and every Session Authority readiness result of `NotReady` or `Missing` MUST produce an operational alert no later than 60 seconds after the triggering observation.
...
**NFR-AUTHORITY-AVAILABILITY-001** — After deployment, over every rolling 365-day window for which 365 days of included operational evidence exist, excluding maintenance recorded as declared before it begins and intervals in which the host, operating system, power, or Controlled LAN is outside its approved healthy condition, the Session Authority process MUST successfully answer an external readiness check from the Controlled LAN every five seconds for at least 99.999 percent of the included checks. Each failed or missing check counts as five seconds of unavailability. This operational target does not gate initial release.
...
| `NFR-OBSERVABILITY-RETENTION-001` | Operational support requirement | Approved; mandatory after deployment | Core signals need a bounded diagnostic window, while the rolling availability calculation needs its complete evidence window | Production observability records; formal acceptance evidence excluded | Inspection of retention configuration and Automated Test of records immediately before and at each expiry boundary | None |
...
| `NFR-OBSERVABILITY-ALERTING-001` | Operational support requirement | Approved; mandatory after deployment | `NFR-OBSERVABILITY-INTEGRITY-001`, `NFR-AUTHORITY-AVAILABILITY-001`; makes measurement loss and Session Authority unavailability visible within a bounded time | Production collectors and availability monitor | Automated Test injecting each trigger and measuring alert creation within 60 seconds | None |
...
| `NFR-AUTHORITY-AVAILABILITY-001` | Operational quality target | Approved; mandatory after deployment; not an initial-release gate | `GOAL-TRAINING-001`; bounds Session Authority process downtime in the controlled deployment without delaying initial release for a year | Deployed Session Authority on its exact approved hardware and deployment profiles once 365 days of included evidence exist | Automated external readiness check every five seconds, plus Analysis of included checks, pre-declared maintenance, healthy-environment exclusions, and failed or missing checks over each rolling window | None |
...
The requirement set was reviewed on 2026-09-01 against the ambiguity checklist in the research guidance. Every normative obligation has a stable identifier, subject, scope, threshold or closed profile reference, priority, rationale, owner, verification method, and pass/fail evidence. Exact build, script, Map, and applicable profile versions are evidence selected and recorded before an acceptance run. No definition-level ambiguity or unresolved acceptance dependency remains; the rolling availability target is explicitly operational and does not gate initial release.

The initial baseline sets no financial limit, delivery deadline, content iteration-time target, or maximum planned-maintenance duration. It requires no infrastructure redundancy, process failover, power redundancy, or network high availability.
